...
''' bias sentence (information bias and lexical bias) identification with event relation graph - BASIL '''
...
triples_0 = range(5, 105, 10)
triples_1 = range(7, 107, 10)
triples_2 = range(6, 106, 10)
triples_3 = range(4, 104, 10)
triples_4 = range(2, 102, 10)
triples_5 = range(9, 109, 10)
triples_6 = range(8, 108, 10)
triples_7 = range(0, 100, 10)
triples_8 = range(3, 103, 10)
triples_9 = range(1, 101, 10)
...
for folder_i in range(10):
    ...
    if nyt_index >= 10:
        nyt_index = nyt_index - 10
    if hpo_index >= 10:
        hpo_index = hpo_index - 10
    if fox_index >= 10:
        fox_index = fox_index - 10

    for triple_index in triples[nyt_index]:
        this_folder.append("./BASIL_event_graph/basil_" + str(triple_index) + "_nyt_event_graph.json")
    for triple_index in triples[hpo_index]:
        this_folder.append("./BASIL_event_graph/basil_" + str(triple_index) + "_hpo_event_graph.json")
    for triple_index in triples[fox_index]:
        this_folder.append("./BASIL_event_graph/basil_" + str(triple_index) + "_fox_event_graph.json")
    ...
    def __getitem__(self, idx):
        ...
        for sent_i in range(len(article_json['sentences'])):
            if len(article_json['sentences'][sent_i]['sentence_text']) > 1: # exclude empty sentence
                ...
                label_sentence.append([start, end, sent_i, article_json['sentences'][sent_i]['label_info_lex_bias']])
...
def evaluate(model, eval_dataloader, verbose):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        decision = torch.argmax(bias_sentence_scores, dim = 1).view(bias_sentence_scores.shape[0], 1)
...
for i in range(-1, 9):
    ...
    for epoch_i in range(num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if step % ((len(train_dataloader) * num_epochs) // check_times) == 0:
                ...
                # evaluate on dev set
                macro_F, biased_F = evaluate(model, dev_dataloader, verbose = 0)
                if biased_F > best_dev_biased_F:
                    torch.save(model.state_dict(), "./saved_models/BASIL/graph_distill_best_dev_biased_F_" + str(test_folder_index) + ".ckpt")
    ...
    model.load_state_dict(torch.load("./saved_models/BASIL/graph_distill_best_dev_biased_F_" + str(test_folder_index) + ".ckpt", map_location=device))
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        if step == 0:
            decision_onetest_biased = decision
            true_label_onetest_biased = true_label
        else:
            decision_onetest_biased = torch.cat((decision_onetest_biased, decision), dim=0)
            true_label_onetest_biased = torch.cat((true_label_onetest_biased, true_label), dim=0)

    decision_onetest_biased = decision_onetest_biased.to('cpu').numpy()
    true_label_onetest_biased = true_label_onetest_biased.to('cpu').numpy()

    print("Macro: ", precision_recall_fscore_support(true_label_onetest_biased, decision_onetest_biased, average='macro'))
    print("Biased: ", precision_recall_fscore_support(true_label_onetest_biased, decision_onetest_biased, average='binary'))
    print("Classification Report \n", classification_report(true_label_onetest_biased, decision_onetest_biased, digits = 4))
    print("======== Finished Testing on test folder: {:} ========".format(test_folder_index))


    if test_folder_index == 0:
        decision_alltests_biased = decision_onetest_biased
        true_label_alltests_biased = true_label_onetest_biased
    else:
        decision_alltests_biased = np.concatenate((decision_alltests_biased, decision_onetest_biased), axis = 0)
        true_label_alltests_biased = np.concatenate((true_label_alltests_biased, true_label_onetest_biased), axis = 0)
...
print("MACRO: ", precision_recall_fscore_support(true_label_alltests_biased, decision_alltests_biased, average='macro'))
print("BIASED: ", precision_recall_fscore_support(true_label_alltests_biased, decision_alltests_biased, average='binary'))
print("Classification Report \n", classification_report(true_label_alltests_biased, decision_alltests_biased, digits = 4))
print("Confusion Matrix \n", confusion_matrix(true_label_alltests_biased, decision_alltests_biased))
